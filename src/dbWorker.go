...
func (confDataBase *DBconfig) DBInsertCodesUsers(codes *Codes) {
	...
	_, err = db.Exec("INSERT INTO CodesUser (Time, NickName,  Code, Danger, Sector) VALUES ($1, $2, $3, $4, $5)",
		codes.Time, codes.NickName, codes.Code, codes.Danger, codes.Sector)
	if err != nil {
		log.Println(err)
	}
}
...
func (confDataBase *DBconfig) DBSelectCodesUser(condition string) []Codes {
	...
	if condition != "" {
		query = fmt.Sprintf("SELECT ID, Time, Team, Hash, NickName FROM Teams WHERE NickName = %s", condition)
	}
...
func (confDataBase *DBconfig) DBCreateTeam(teams *Teams) string {
	...
	_, err = db.Exec("INSERT INTO Users (NickName, Time, Team) VALUES ($1, $2, $3)",
		teams.NickName, teams.Time, teams.Team)
	if err != nil {
		log.Println(err)
		return err.Error()
	}
