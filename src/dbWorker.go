...
func (dbConfig *Config) DBCreateTables() string {
	...
	sqlCommand := `
		CREATE TABLE CodesRight_test(
			ID		integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
			Code    varchar(300) UNIQUE NOT NULL,
			Danger	varchar(50) NOT NULL,
			Sector	varchar(100) NOT NULL);
		CREATE INDEX ON CodesRight_test(Code text_pattern_ops);
		...
		CREATE TABLE Teams_test(
			...
			UserID		INTEGER);
		CREATE INDEX ON Teams_test(Team text_pattern_ops);
	
		CREATE TABLE CodesUser_test(
			ID			integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
			Time		varchar(50) NOT NULL,
			NickName	varchar(100) NOT NULL,
			UserID		INTEGER NOT NULL,
			Code		varchar(300) REFERENCES CodesRight_test (Code) ON DELETE CASCADE,
			Team		varchar(100) REFERENCES Teams_test (Team) ON DELETE SET NULL);
		CREATE INDEX ON CodesUser_test(NickName text_pattern_ops);
		...
		CREATE INDEX ON Users_test(UserID);
	`
...
func (dbConfig *Config) DBTruncTables(name string) string {
	...
	maps["teams"] = `truncate table Teams_test CASCADE;`
	maps["codes"] = `truncate table CodesRight_test CASCADE;`

	_, err = db.Exec(maps[name])
	if err != nil {
		return fmt.Sprintf("ERROR delete table %s: %s", name, err)
	}

	return fmt.Sprintf("\n\n&#9940;Таблица %s удалена", name)
}
