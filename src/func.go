...
func CreateTable(name string, dbConfig Config) string {
	var tables = []Tables{
		{"Teams", "CREATE TABLE " +
			"Teams(" +
	...
	for key, value := range tables {
		if value.name == name {
			return dbConfig.DBResetTable(tables[key])
		}
	}
	return "&#8252;Неверное имя БД"
}
