	...
	for number, valueRight := range dataRight {
		...
		for _, valueUser := range dataUser {
			if valueUser.Code == valueRight.Code {
				str += fmt.Sprintf("%d. КО: <b>%s</b>, сектор <b>%s</b>, &#9989;<b>СНЯТ</b> (%s), бонус <b>%s</b>, задание <b><a href=\"/task %d\">%d</a></b>\n", number+1, valueRight.Danger, valueRight.Sector, valueRight.Code, ConvertTimeSec(valueRight.TimeBonus), valueRight.TaskID, valueRight.TaskID)
				isFound = true
				break
			}
		}
