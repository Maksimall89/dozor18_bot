...
func ShowCodesAll(dbConfig Config) string {
	...
	for number, value := range dataAllRight {
		str += fmt.Sprintf("%d. <b>Код:</b> %s; <b>КО:</b> %s; <b>Сектор:</b> %s; <b>Бонус:</b> %d;\n", number+1, value.Code, value.Danger, value.Sector, value.TimeBonus)
	}
